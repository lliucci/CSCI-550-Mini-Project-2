...
For best subset selection, all models were searched using the `dredge` function within `R` and compared using AIC as a criterion. The model determined to be best is shown below.

```{r}
# Fit model determined by dredge
################# insert dredge model ######################
...
# Predict to test data
bss_preds = predict(bss, test)
print(paste("MSE from Test Set: ", Metrics::mse(bss_preds, test$Sale_Price)))
```

# Please put Ridge and Lasso Stuff Here
...
print(paste("MSE from Test Set: ", Metrics::mse(ridge_pred, test$Sale_Price)))
# model coefficients
...
print(paste("MSE from Test Set: ", Metrics::mse(lasso_pred, test$Sale_Price)))
# model coefficients
...
Comparing the models on mean squared error when predicting to the test set, we can see which model performed the best when predicting to the test data (Table \@ref(tab:MSETable)).

```{r MSETable, eval = F}
tibble(Model = c("SLR", "MLR", "BSS", "Ridge", "Lasso", "PCA", "Polynomial"),
    MSE = c(21567219262.6313, 10429798940.6481, "BSS", "Ridge", "Lasso", 37437607836.8467, 21249303121.8831)) %>%
    arrange(desc(MSE)) %>%
